...
##        vsum += dot_product
#        
#        if wid in v0 and wid in v1:
#            dot_product = np.multiply((v0[wid]),(v1[wid]))
#            vsum += dot_product
#        else:
#            pass
#    for wid in range(len(wid2word)):
...
#        else:
#            pass
##    vlength = np.multiply(sqrt(unrooted0),sqrt(unrooted1))
##    normalised_cos_sim = np.divide(vsum,vlength)
##    return normalised_cos_sim
#
#    keys = v0.keys() & v1.keys()
##    numerator = sum(v0[k] * v1[k] for k in keys)
#    v0mag = sqrt(sum(v0[k]**2 for k in v0.keys()))
#    v1mag = sqrt(sum(v1[k]**2 for k in v1.keys()))
##    return np.divide(numerator, np.multiply(v0mag, v1mag))
#    
#    return np.divide(vsum, np.multiply(v0mag, v1mag))




testvec1 = {0:0, 1:0, 2:1, 3:1}
testvec2 = {0:0, 1:4, 2:0, 3:2}
print('test cos sim: ', cos_sim(testvec1, testvec2))
...
def print_sorted_pairs(similarities, o_counts, first=0, last=100):
    ...
    with open('c_sims_PPMI.txt', 'w') as f:
        pair_freq = []
        word1_freq = []
        word2_freq = []
        pair_similarities = []
        ...
        for pair in sorted(similarities.keys(), key=lambda x: similarities[x], reverse = True)[first:last]:
            ...
            #pair_freq.append(co_counts[word2wid[pair[0]][word2wid[pair[1]]]])
        ...
        f.write(min_freq_v_sim(c_sims))
        f.write(max_freq_v_sim(c_sims))
        f.write(diff_in_freq_v_sim(c_sims))
    '''
...
def min_freq_v_sim(sims):
  ...
  ourplot.savefig('PPMI_c_sim_min.pdf')
  return ("\nFreq vs Similarity Spearman correlation = {:.2f}".format(stats.spearmanr(xs,ys)[0]))
...
def max_freq_v_sim(sims):
  ...
  plt.show() #display the set of plots
  ourplot.savefig('PPMI_c_sim_max.pdf')
  return ("\nFreq vs Similarity Spearman correlation = {:.2f}".format(stats.spearmanr(xs,ys)[0]))
...
def diff_in_freq_v_sim(sims):
  ...
  plt.show() #display the set of plots
  ourplot.savefig('PPMI_c_sim_diff.pdf')
